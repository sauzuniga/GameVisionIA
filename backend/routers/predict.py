...
def predict(
    ...
    try:
        ...
        db.add(chat_session)
        db.flush()
        # Leemos el id AQUÍ, antes del commit. SessionLocal tiene
        # expire_on_commit=True (el default), así que si lo leyéramos
        # después de db.commit(), SQLAlchemy dispararía un SELECT nuevo
        # por debajo para "refrescar" el objeto — justo el viaje extra
        # que queremos evitar.
